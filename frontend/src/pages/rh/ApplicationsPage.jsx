...
import { Spinner, StatusBadge, EmptyState, Button, Avatar, Pagination } from "components/ui";
...
export default function ApplicationsPage() {
  ...
  const charger = (p = 1) => {
    const params = { tri, page: p, page_size: PAGE_SIZE };
  ...
  useEffect(() => {
    offresAPI.lister({ page_size: 100 }).then(r => setOffers(r.data.items));
    charger(1);
  }, []);

  useEffect(() => { setLoading(true); charger(1); }, [fOffer, fStatus, tri]);

  const handlePage = p => { setLoading(true); charger(p); };
          ...
          <div className="table-wrap">
            <table>
              ...
              <tbody>
                {apps.map((a, i) => (
                  <tr key={a.id}>
                    <td style={{ color: "var(--text3)", fontSize: 12 }}>{(page - 1) * PAGE_SIZE + i + 1}</td>
                    <td>
