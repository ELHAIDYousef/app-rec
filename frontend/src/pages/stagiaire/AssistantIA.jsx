      ...
      <div style={{ borderTop: "1px solid var(--gray2)", paddingTop: 14 }}>
        <div style={{ display: "flex", gap: 8, alignItems: "flex-end" }}>
          <textarea
            ...
            rows={3}
            placeholder="Tapez votre message... (Entrée pour envoyer, Maj+Entrée pour sauter une ligne)"
            value={input}
            onChange={e => setInput(e.target.value)}
            onKeyDown={e => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); envoyer(); } }}
            style={{ flex: 1, resize: "none", fontSize: 14, padding: "12px 14px", minHeight: 72 }}
          />
          <button className="btn btn-primary" onClick={envoyer} disabled={!input.trim() || loading}
            style={{ minWidth: 80, whiteSpace: "nowrap" }}>
            {loading ? "●●●" : "Envoyer"}
          </button>
        </div>
        {messages.length > 0 && (
          <button className="btn btn-outline btn-sm" style={{ marginTop: 10 }} onClick={vider}>
...
export default function AssistantIA() {
  ...
  return (
    <div>
      ...
      <div className="card" style={{ minHeight: 520, display: "flex", flexDirection: "column" }}>
