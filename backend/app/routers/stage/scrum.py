...
from app.models.stage import CandidatureStage, Sprint, Tache, DailyReport, CahierDesCharges, SujetStage
...
def mes_reports(
    ...
    user=Depends(require_role("stagiaire", "admin", "rh", "encadrant")),
):
    if user.role == "stagiaire":
        q = db.query(DailyReport).filter(DailyReport.stagiaire_id == user.id)
    elif user.role == "encadrant":
        sujet_ids = db.query(SujetStage.id).filter(SujetStage.encadrant_id == user.id).subquery()
        stag_ids = db.query(CandidatureStage.stagiaire_id).filter(
            CandidatureStage.sujet_id.in_(sujet_ids)
        ).subquery()
        q = db.query(DailyReport).filter(DailyReport.stagiaire_id.in_(stag_ids))
    else:
